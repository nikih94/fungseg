...
- `patch_cache.py`: run-level static-region cache construction, mask-only epoch crop planning, and lazy uint8 NumPy-memmap final-crop loading; runtime transforms and target weighting remain outside the cache.
...
- `trainer.py`: fold training without hard-clDice, stitched full-image Dice/IoU/Zhang hard-clDice validation with cached target skeletons, foreground-only validation soft-clDice, phase timing, per-batch finite checks, scheduler stepping, checkpointing, TensorBoard logging, and checkpoint manifests.
...
3. `patching/core.py` creates source records and deterministic phase geometry; `data/patch_cache.py` creates static training regions.
...
7. `trainer.py` begins stitched full-image validation at `validation.start_epoch`. Each batched forward supplies both patch loss and stitching probabilities. CE and Dice cover all patches; configured validation soft-clDice may select only target-foreground samples. The combined stitched Dice/hard-clDice score drives `best_current.pt`, and patch-count-weighted `val_loss` drives plateau schedulers and `best_val_loss.pt`. Training omits hard-clDice, while full-image validation caches each immutable target skeleton and computes each stitched prediction skeleton once per epoch for reuse by all hard-clDice monitors.
