...
- `multiclass-config-1080.yaml`: the related 1080-oriented multiclass U-Net++/ResNet50 experiment.
...
python -m src.train --config multiclass-config-1080.yaml
...
`validation.full_image` reconstructs each selected source with a fixed stride of `patching.patch_size // 2` and runs image patches in groups of `batch_size`. Every overlap patch is inferred exactly once and all probabilities are stitched. `val_loss` reuses logits from that same forward pass but selects only the strict disjoint starts `0, patch_size, 2 * patch_size, ...` on each axis while a full patch fits; a source no larger than one patch contributes one padded loss patch. Final right/bottom edge patches that overlap the disjoint grid remain part of stitching but not loss. No foreground threshold or `min_foreground_pixels` filter is applied, so background-only disjoint patches remain in `val_loss`. Loss aggregation is weighted by the number of selected loss patches in each inference batch. `validation.full_image.loss.patch_selection` records this `non_overlapping` policy. `selection: smallest_area` with positive `max_images` selects a deterministic area-sorted source subset (ties use source ID); `selection: all` uses every validation source. When the patch cache is enabled, all selected RGB grid patches are cached, but target patches and adaptive loss metadata are stored only for the disjoint loss subset. Training requires `enabled: true` and `interval_epochs: 1` so checkpoint and scheduler monitors are available on every validation epoch.

Validation reports stitched Dice, IoU, and fast Zhang hard-clDice alongside diagnostic `val_loss`. The immutable target skeleton is cached per source for the fold, while each stitched prediction is skeletonized once per validation epoch and reused by every hard-clDice-based monitor. For multiclass validation, `val_dice_per_image` is the arithmetic mean of stitched loci and inoculum Dice. Named formulas under `validation.full_image.composite_metrics` expose each metric and its weights; checkpoint and scheduler `monitor` fields reference the resulting `val_<name>`. When `validation.full_image.soft_cldice_foreground_only: true`, every disjoint patch still enters CE and Dice, while differentiable soft-clDice runs only for samples whose target contains loci/class 1 (or positive target pixels in binary mode). Training soft-clDice is unmasked.

Checkpoint creation is controlled by `checkpointing.selections`: each named entry has `enabled`, `filename`, `monitor`, and `mode`, and `checkpointing.primary` identifies the selection used for fold-level summaries and interval-best files. The checked-in multiclass configs enable four hard-mask selections (`best_current.pt`, `best_dice.pt`, `best_low_cldice.pt`, and `best_inoculum_compensated.pt`); binary configs enable `best_current.pt` and `best_dice.pt`. A visible `validation_loss` selection remains disabled by default because `val_loss` is diagnostic. `checkpointing.save_last` and `checkpointing.interval` replace their legacy `train.*` counterparts, which remain load-compatible. Multiclass loss-component logging reuses tensors from the actual loss calculation. `train.compute_hard_cldice_metrics: false` disables expensive per-training-batch CPU skeleton diagnostics without disabling validation hard-clDice. Checked-in `ReduceLROnPlateau` schedulers maximize `val_dice_cldice_per_image`, use an absolute improvement threshold of `0.001`, and remain idle before `validation.start_epoch`. No validation-selected checkpoint exists until the first validation pass completes.
...
With static caching and a Soft-clDice loss, `loss.static_patch_iterations.enabled: true` computes the exact required count once from each cached loss target. Training uses the larger static region, making its value a conservative bound for every random epoch crop. Cached validation computes it only for each strict disjoint loss patch; overlap-only patches require no target metadata. For multiclass targets both paths use effective loci after optional join merging and inoculum precedence. `margin_iterations` is added and the result is rounded upward to `round_up_to`; empty targets receive the rounded margin. A configured `iterations_csv` remains the per-source override, and `loss.iterations` remains the fallback when adaptive metadata is unavailable.
...
- `run_state.json`, optional `resume_history.json`, `cv_summary.json`, `fold_metrics.csv`, and `epoch_metrics.csv`; epoch rows include `train_duration_seconds` (epoch patch planning, loader lifecycle, and training) and `validation_duration_seconds` (cached/direct patch inference, stitching, and metrics, excluding cache construction and checkpoint writes);
- one `fold_<n>/` directory per split, containing the enabled `checkpointing.selections` (four in checked-in multiclass configs and two in binary configs), optional `last.pt` and interval-best checkpoints according to `checkpointing.save_last` and `checkpointing.interval`, metric history, `patch_distribution.json` (including FIVES, training-cache, validation-cache, adaptive-iteration, and validation-source diagnostics), TensorBoard logs, and checkpoint manifests; fold metric and manifest CSV/JSON files are refreshed after every completed epoch; and
- `test-evaluation/` when a CSV-backed test split is evaluated. Checkpoint selections that resolve to the same epoch share one inference under `fold_<n>/epoch_<epoch>/` (or `epoch_<epoch>/` for one fold). Root `checkpoint_comparison.csv` directly compares every fold/checkpoint pair, while `monitor_comparison_summary.csv` reports each monitor’s cross-fold means, population standard deviations, and contributing-fold counts. Per-fold `checkpoint_comparison.csv` files are also written. The existing root `fold_metrics.csv`, `summary.csv`, and `summary.json` remain the cross-fold summary for the configured primary checkpoint.
